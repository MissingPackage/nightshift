---
name: brainstorming
description: "Use before creative work whose requirements or design are not yet agreed — new features, new components, behavior changes with open questions. Skip when an approved spec or ruling already fixes the design. Explores user intent, requirements and design before implementation."
---
<!-- vendored: superpowers v6.3.0 (github.com/obra/superpowers). License: MIT — see
     skills/vendored/LICENSE-superpowers. Local copy = no auto-updates; re-diff periodically.
     Three deliberate local adaptations are marked "nightshift drift" below. -->
<!-- nightshift drift 1/3 — description retuned. Upstream reads "You MUST use this before any
     creative work". Recent Claude models over-trigger on "You MUST … any" phrasing and invoke
     the skill inside work that is already past the design gate. The trigger is now descriptive,
     and the HARD-GATE carries an explicit exception instead. -->
...
**Terminal states are path-bound.** Architectural: the next skill is a
plan-shaped one — never frontend-design, mcp-builder, or any other
...
- Invoke the plan-shaped skill the scale fork selected above — `writing-plans` for
  feature-sized work, `spec-first` + `/goal-brief` for project-sized work.
- Do NOT invoke any other skill. That one is the next step.
